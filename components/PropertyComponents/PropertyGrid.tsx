...
const PropertyGrid = async ({className} : {className:string}) => {
 ...
 console.log(getProperty.properties[0]);
  return (
    <div className={className}>
        {getProperty.properties.map((property : any)=> {
         ...
         return <PropertyCard key={propertyId} propertyId={propertyId} imageUrl={images} name={name} city={city} state={state} slug={slug} rating={5} guests={maxOccupancy} baths={5} rooms={5} pricePerNight={baseAmount} currency={currency}/>
        })}
    </div>
  )
}
